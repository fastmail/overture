// In Overture, we need `init` or `constructor` to *not* use object shorthand,
...
module.exports = {
    ...
    create(context) {
         ...
         * @private
         */
        function looseStartsWithUppercase(name) {
        ...
        let isConstructor = CONSTRUCTORS && CONSTRUCTORS.pattern || looseStartsWithUppercase;

        if (typeof isConstructor === "string") {
            const pattern = new RegExp(isConstructor);
            isConstructor = (name) => pattern.test(name);
        }
        ...
        return {
            ...
            "Property:exit"(node) {
                ...
                if (isConciseProperty) {
                    if (node.method) {
                        const causeConstructor = CONSTRUCTORS && !CONSTRUCTORS.ignore && node.key.type === "Identifier" && isConstructor(node.key.name);
                        if (causeConstructor || APPLY_NEVER || AVOID_QUOTES && isStringLiteral(node.key)) {
                            const message = APPLY_NEVER
                                ? "Expected longform method syntax."
                                : causeConstructor
                                ? "Expected longform method syntax for constructor."
                    ...
                    if (CONSTRUCTORS && node.key.type === "Identifier" && isConstructor(node.key.name)) {
                        return;
                    }
