...
const runeAt = (string, index) => {
    ...
    code = string.charCodeAt(index + 1);
...
/**
    ...
    Parameters:
        string - {String} The string
        args - {...(String|Number|Object)} The arguments to interpolate.
    ...
    while ((part = splitter.exec(string))) {
        ...
        switch (part[7]) {
            ...
            case 'n':
                ...
                toInsert +=
                    part[5] !== undefined ? data.toFixed(part[5]) : data + '';
                break;
            case '@':
...
/**
    ...
    Returns the string with the first letter converted to a capital.
...
/**
    ...
    Returns the string with any sequence of a hyphen followed by a
...
/**
    ...
    Returns the string with any captials converted to lower case and
...
*/
const hash = (string) => {
    let int32 = string.length;
    const remainder = int32 & 1;
    const l = int32 - remainder;

    for (let i = 0; i < l; i += 2) {
        int32 += string.charCodeAt(i);
        int32 = (int32 << 16) ^ ((string.charCodeAt(i + 1) << 11) ^ int32);
        int32 += int32 >> 11;
    }

    if (remainder) {
        int32 += string.charCodeAt(l);
        int32 ^= int32 << 11;
        int32 += int32 >> 17;
    }
